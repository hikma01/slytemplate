import {composePresentationTree, type CoreDesignInputs} from '../../core/index.js';
import {evaluateRenderFeedback, type FeedbackEvaluationContext} from '../../feedback/index.js';
import {ChromiumBackend, type ChromiumBackendConfiguration} from '../../renderer-adapter/chromium/index.js';
import {RendererAdapter, type RendererAdapterResult} from '../../renderer-adapter/index.js';
import type {ImmutableVersionRef, LineageEntry, PresentationDocumentDTO, RenderFeedback, RenderRequest, Slot} from '../../contracts/index.js';
...
const slot = (id: string, type: Slot['type'], content: unknown, items: readonly unknown[], semantics: string): Slot => ({id, type, exists: true, items, count: items.length, content, semantics});

export function adaptSlyCvProducerInput(input: SlyCvProducerInput): PresentationDocumentDTO {
  ...
  if (input.summary !== undefined) slots.summary = slot('summary', 'RICH_TEXT', input.summary, [], 'producer-supplied summary');
  if (input.education !== undefined) slots.education = slot('education', 'ITEM_LIST', null, input.education, 'producer-supplied education entries');
  if (input.certifications !== undefined) slots.certifications = slot('certifications', 'ITEM_LIST', null, input.certifications, 'producer-supplied certification entries');
  if (input.languages !== undefined) slots.languages = slot('languages', 'ITEM_LIST', null, input.languages, 'producer-supplied language entries');
  return {
...
function requestFor(document: PresentationDocumentDTO, input: SlyCvProducerInput, requestId: string): RenderRequest {
  return {
...
function feedbackResultForFixture(result: RendererAdapterResult, input: SlyCvProducerInput): RendererAdapterResult {
  if (result.measurements === null) return result;
...
export async function renderSlyCvEndToEnd(input: SlyCvProducerInput, chromiumConfiguration: ChromiumBackendConfiguration, requestId = `render-${input.documentId}`): Promise<CvEndToEndResult> {
  ...
  const context: FeedbackEvaluationContext = {presentationRunId: `run-${input.documentId}`, attempt: 1, createdAt: '2026-09-09T00:00:00.000Z'};
  return {input, document, request, tree, render, feedback: evaluateRenderFeedback(render, context)};
}

export function createCvFixture(id: string): SlyCvProducerInput {
  const definitions: Record<string, {name: string; title: string; summary: string; experienceCount: number; skillCount: number; educationCount: number; paragraphs: number; unicode?: boolean; constraint?: boolean}> = {
  ...
  if (definition === undefined) throw new Error(`Unknown CV fixture ${id}`);
