...
test('executes the six SlyCV producer-to-Chromium fixtures', {skip: !existsSync(executablePath)}, async () => {
  let unicodeSignature: string | undefined;
  for (const [fixtureId, expectedStatus] of expected) {
    ...
    if (fixtureId === 'CV_UNICODE') unicodeSignature = JSON.stringify({status: result.feedback.status, pages: result.render.measurements?.page_count, measurements: result.render.measurements});
