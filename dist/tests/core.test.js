...
const slot = (id, content = `${id}-content`) => ({ id, type: 'TEXT', exists: true, items: [], count: 0, content, semantics: id });
const request = { request_id: 'req-1', traceId: 'trace-1', contextId: 'ctx-1', lineage: [{ source_id: 'source-1', transform_id: 'projection-1', version }], template_id: 'cv-1', template_version: '1.0.0', payload: { document_id: 'doc-1', contract_version: '1.0.0', artifact_kind: 'CV', locale: 'fr-FR', slots: { summary: slot('summary'), optional: { ...slot('optional'), exists: false, content: null } }, content_lineage: [] }, presentation_contract_version: '1.0.0', design_policy_version: version, token_set_version: { reference: 'tokens', version: '1.0.0' }, token_overrides: { accent: 'blue' }, presentation_preferences: {}, renderer_configuration_ref: null };
...
const inputs = { token_set: { reference: 'tokens', version: '1.0.0' }, tokens: { accent: 'red' }, design_policy: { reference: 'policy', version: '1.0.0' }, policies: { density: 'standard' } };
...
test('rejects an incompatible slot type', () => { const invalid = { ...request, payload: { ...request.payload, slots: { ...request.payload.slots, summary: { ...slot('summary'), type: 'GROUP' } } } }; rejects(() => composePresentationTree(invalid, template, inputs), 'INCOMPATIBLE_SLOT_TYPE'); });
...
test('rejects a template contract version outside its range', () => { const invalid = { ...request, presentation_contract_version: '2.0.0' }; rejects(() => composePresentationTree(invalid, template, inputs), 'INCOMPATIBLE_CONTRACT_VERSION'); });
